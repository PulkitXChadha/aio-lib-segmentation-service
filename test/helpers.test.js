...
describe("reduceError", () => {
  ...
  test("err is undefined", () => {
    ...
    const received = reduceError(mockErr);
  ...
  test('err is {response: {status: 200, statusText: "ok"}}', () => {
    ...
    const received = reduceError(mockErr);
    expect(received).toEqual({ response: { status: 200, statusText: "ok" } });
  });
  test("err is {response: {body: {fake: string }}}", () => {
    const mockErr = { response: { body: { fake: "string" } } };
    const received = reduceError(mockErr);
    expect(received).toEqual({ response: { body: { fake: "string" } } });
  });
  test('err is {response: {status: 200, statusText: "ok" body: {fake: "string" }}}', () => {
    const mockErr = {
      response: { status: 200, statusText: "ok", body: { fake: "string" } },
    };
    const response = mockErr.response;
    const received = reduceError(mockErr);
    expect(received).toEqual(`200 - ok (${JSON.stringify(response.body)})`);
  });
});
