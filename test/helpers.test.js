...
test("reduceError", () => {
  ...
  expect(reduceError(expectedError)).toEqual(
    '500 - Something went gang aft agley. ({"error_code":500101,"message":"I\'m giving it all I got, cap\'n"})'
  );
});
...
test("responseBodyToString", async () => {
  ...
  res = new fetchMock.Response(body);
  await expect(responseBodyToString(res)).resolves.toEqual(body);

  // error coverage
  res = { text: () => {} };
  await expect(responseBodyToString(res)).rejects.toEqual(
